 ...
 * 各 profile 的 ModelConfig 装配 {@code LlmCapabilities} Bean，{@code CapabilitiesController}
 * 注入并透传——同一启动期内仅一个 Bean 装配（{@code @ConditionalOnProperty} 保证）。
 *
 * <p>v0.135 降级现状：当前 deepseek-v4 / minimax 两个 profile 的 thinkingToggle / thinkingBudget
 * 均为 false。LC4J 1.13.1 的 langchain4j-anthropic 没有 AnthropicChatRequestParameters 子类，
 * thinking 参数只能 builder 期固定，前端按钮统一仅控显示。等上游解除时，仅需改 ModelConfig
